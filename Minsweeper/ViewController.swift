...
class ViewController: UIViewController {
    ...
    let BOARD_SIZE:Int = 5
    var board:Board
    var squareButtons:[SquareButton] = []
    
    var moves:Int = 0 {
        didSet {
            self.movesLabel.text = "\(moves) :چالوں کی تعداد"
            self.movesLabel.sizeToFit()
        }
    }
    var timeTaken:Int = 0  {
        didSet {
            self.timeLabel.text = "\(timeTaken) :وقت"
...
//        sender.setTitle("", forState: .Normal)
    ...
    func minePressed() {
        ...
        alertView.addButtonWithTitle("نیا کھیل")
        alertView.title = "زور دار دھماکا!"
        alertView.message = "آپ نے ايک بادودی سرنگ پر قدم رکھ ديا ہے"
        alertView.show()
